...
{
 feedbackValueh = analogRead(feedbackPinh);
    angle=map(feedbackValueh,40,517,-110,140);
  //  angle1=map(feedbackValueh,40,517,200,-50);
    feedbackValuev = analogRead(feedbackPinv);
    altitude=map(feedbackValuev,127,492,8,120); 
 ...
 // Serial.println(" ");
  ...
  Serial.print(avd);
  Serial.println(" ");*/
  while (millis() - start_time <= 981) {               //981 and up until 990
